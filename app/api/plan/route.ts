...
function userId(request: Request) {
  return request.headers.get("oai-authenticated-user-id") || "daniel-family";
}
...
export async function GET(request: Request) {
  try {
    ...
    let row = await database.prepare("SELECT payload, updated_at FROM user_plan_state WHERE user_id = ?")
      .bind(userId(request)).first<{ payload: string; updated_at: string }>();
    if (!row) {
      try {
        const legacy = await database.prepare("SELECT payload, updated_at FROM plan_state WHERE id = ?")
        ...
        if (legacy) {
          await database.prepare("INSERT INTO user_plan_state (user_id, payload, updated_at) VALUES (?, ?, ?)")
            .bind(userId(request), legacy.payload, legacy.updated_at).run();
          row = legacy;
        }
      } catch {
...
export async function POST(request: Request) {
  try {
    ...
    ).bind(userId(request), JSON.stringify(payload), updatedAt).run();
